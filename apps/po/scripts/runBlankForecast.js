...
(async () => {
    ...
    console.log(`[blanks-forecast] po starting at ${new Date().toISOString()}`);
    try {
        const res = await fetch(url, { method: "POST" });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        if (data.error) throw new Error(data.msg);
        console.log(`[blanks-forecast] po done — ${data.needsReorderCount} SKUs need reorder, est. order value $${(data.totalOrderValue || 0).toFixed(2)} (${Date.now() - start}ms)`);
    } catch (e) {
        console.error(`[blanks-forecast] po error: ${e.message}`);
