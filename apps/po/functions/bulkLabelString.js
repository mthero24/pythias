...
export const buildBulkLabelData = async (item, i, poNumber, opts={}) => {
    ...
    let labelString = `^XA
        ^FO50,80^BY2^BC,120,N,N,N,A^FD${item.bulkId}^FS
        ^LH6,6^CFS,30,6^AXN,22,30^FO15,30^FDPO#: ${
            item.order ? item.order.poNumber : "no order"
        }^FS
        ^LH6,6^CFS,30,6^AXN,22,30^FO15,60^FDBulk ID: ${item.bulkId}^FS
        ^LH12,18^CFS,25,12^AXN,22,30^FO20,330^FD#${i + 1}^FS
        ^LH12,18^CFS,25,12^AXN,75,90^FO120,250^FD${item.blankCode}^FS
        ...
        ^LH12,18^CFS,25,12^AXN,22,30^FO20,500^FDCNT: ${item.totalQuantity}^FS
        ^LH12,18^CFS,25,12^AXN,22,30^FO150,500^FDQTY: ${item.quantity}^FS
        ${
            printTypeAbbr
