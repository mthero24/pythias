...
export default async function TrackLabels(){
    try{
        ...
        items = items.filter(item => item.order && item.order.status !== "canceled" && item.order.status !== "returned" && item.order.status !== "shipped" && item.order.status !== "Shipped" && item.order.status !== "Delivered" && item.order.status !== "Complete");
