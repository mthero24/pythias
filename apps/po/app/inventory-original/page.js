...
const Index = (props) => {
    ...
    const getInventoryValue = async () => {
      let results = await axios.get("/api/admin/inventory-original/get-cash-value", {});
    ...
    const updatePendingOrders = () => {
      axios.post("/api/admin/inventory-original/update-pending-orders");
    };
  
    const placeInventoryOrder = async (item) => {
      let result = await axios.post("/api/admin/inventory-original/orders", {
        inOrder,
        po: item,
      });
      console.log(result);
    ...
    const getInventory = async (page = 0) => {
      console.log("getInve");
      let results = await axios.get("/api/admin/inventory-original", {
  ...
  const EditModal = ({
    ...
    const handleClick = async () => {
      let result = await axios.post("/api/admin/inventory-original", {
        quantity,
        cost,
        aisle,
        shelf,
    ...
    const handleDelete = async () => {
      if (confirm("Are you sure you want to delete this inventory item?")) {
        let result = await axios.put("/api/admin/inventory-original",  {
            inventory_id: activeItem.inventory_id,
          },
        );
        setModalVisible(false);
