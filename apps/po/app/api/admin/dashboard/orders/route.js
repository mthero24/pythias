...
export async function GET(req) {
    try {
        ...
        const rawItems  = await Items.find({ order: { $in: orderIds }, canceled: { $ne: true } }).select("order styleCode sizeName").lean();
        const itemsWithCogs = await addCogs(rawItems);

        const cogsByOrder = {};
        for (const i of itemsWithCogs) { const key = String(i.order); cogsByOrder[key] = (cogsByOrder[key] || 0) + (i.wholesaleCost || 0); }

        const enriched = orders.map(o => ({ ...o, blanksCogs: cogsByOrder[String(o._id)] || 0, shippingCost: shipCostExpr(o) }));
        ...
        if (csvMode) {
            ...
            const rows = enriched.map(o => [
                ...
                (o.productCost ?? 0).toFixed(2), (o.shippingCost ?? 0).toFixed(2), (o.blanksCogs ?? 0).toFixed(2),
            ]);
