...
import BlankForecastCache from "@/models/BlankForecastCache";
...
async function computeForecast() {
    ...
    const [styles, invDocs] = await Promise.all([
        StyleV2.find({ code: { $in: styleCodes } }).select("code sizes").lean(),
    ...
    for (const s of styles) {
        costMap[s.code] = {};
        for (const sz of s.sizes ?? []) costMap[s.code][sz.name] = sz.wholesaleCost ?? 0;
    }
    const invMap = {};
...
export async function GET() {
    try {
        const cached = await BlankForecastCache.findOne().select("-_id -__v").lean();
...
export async function POST() {
    try {
        ...
        await BlankForecastCache.findOneAndUpdate({}, { ...result, computedAt }, { upsert: true, new: true });
