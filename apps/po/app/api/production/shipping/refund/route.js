...
export async function PUT(req= NextApiRequest){
    ...
    let orders = await Order.find({"shippingInfo.labels.delivered": {$in: [false, undefined]}, date: {$gt: new Date(Date.now() - 60 * (24 * 60 * 60 * 1000))}, "selectedShipping.provider": "usps"}).select("shippingInfo date poNumber").sort({date: 1}).skip(data.page * 50 - 50).limit(50)
    return NextResponse.json({error: false, orders})
}