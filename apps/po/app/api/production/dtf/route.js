...
export async function GET(req = NextApiRequest) {
  ...
  let pieceID;
  let item;
  if (req.nextUrl.searchParams.get("pieceID"))
    pieceID = req.nextUrl.searchParams.get("pieceID");
  if (pieceID)
    item = await Items.findOne({ pieceId: pieceID }).populate(
      "styleV2",
      "code sizes images",
    );
  console.log(item);
...
export async function POST(req = NextApiRequest) {
  ...
  if (!item) {
    ...
    let quantity = await Items.find({
      bulkId: data.pieceId.toUpperCase().trim(),
    }).countDocuments();
    let style = await Style.findOne({ _id: items[0].styleV2._id }).select(
      "code envelopes box sizes images",
    );
        ...
        Object.keys(item.design).map(async (im) => {
          ...
          if (im && im != "") {
            ...
            let envelope = item.styleV2.envelopes.filter(
              (ev) =>
                (ev.sizeName == item.sizeName ||
                  ev.size?.toString() == item.size?.toString()) &&
                im == ev.placement,
            )[0];
            if (!envelope)
              envelope = item.styleV2.envelopes.filter(
                (ev) => im == ev.placement,
              )[0];
    ...
    item.dtfScan = true;
    let style = await Style.findOne({ _id: item.styleV2._id }).select(
      "code envelopes box sizes images",
    );
    item.styleV2 = style;
    let shouldFitDesign = item?.styleV2?.box?.default?.front?.autoFit;
    ...
    return NextResponse.json({
      error: true,
      msg: "Item Already Scanned Into DTF Scan Again To Resend",
      design: item.design,
    });
  } else {
...
export async function PUT(req = NextApiRequest) {
  ...
  try {
    let items = await Items.find({
      ...
      .populate("inventory.inventory inventory.productInventory")
      .populate("order", "poNumber items marketplace date")
      .populate("blank", "code envelopes box sizes multiImages");
    console.log(items.length, "items to print");
    items = items.filter(
      (i) =>
        i.inventory?.inventory &&
        i.inventory?.inventory?.inventoryType == "inventory" &&
        i.inventory?.inventory?.inventory.inStock.includes(i._id.toString()),
    );
          ...
          Object.keys(item.design).map(async (key) => {
            if (key != undefined && item.design[key]) {
              let envelopes = item.blank.envelopes.filter(
                (envelope) =>
                  (envelope.size?.toString() == item.size.toString() ||
                    envelope.sizeName == item.sizeName) &&
                  envelope.placement == key,
              );
              await createImage({
                url: item.design[key],
