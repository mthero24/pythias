...
export default function AccountButton() {
    ...
    const base = {
        ...
        background: "var(--sf-bg, #ffffff)", color: "var(--sf-text, #111111)", fontWeight: 700,
        padding: "8px 14px", borderRadius: 999, boxShadow: "0 2px 10px rgba(0,0,0,.18)",
        fontSize: "0.9rem", textDecoration: "none", border: "1px solid rgba(128,128,128,0.25)",
    };

    if (!customer) {
        return <a href="/account/login" style={base}>👤<span className="sf-acct-label">&nbsp;Sign in</span></a>;
    }

    const label = customer.name?.trim()?.split(" ")[0] || "Account";
    return (
        <a href="/account" style={base} title={customer.email}>
            👤<span className="sf-acct-label">&nbsp;{label}</span>
