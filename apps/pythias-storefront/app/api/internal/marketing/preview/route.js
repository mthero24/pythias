...
export async function POST(req) {
    ...
    const { orgId, channel, subject, html, body, to } = await req.json().catch(() => ({}));
    ...
    if (!subject || !html) return NextResponse.json({ error: "Email subject and content are required" }, { status: 400 });
    const site = orgId ? await StorefrontSite.findOne({ orgId, plan: { $ne: "none" } }).select("name").lean() : null;
    const r = await sendEmail({ to, subject: `[Preview] ${subject}`, html, from: brandedFrom(site?.name) });
