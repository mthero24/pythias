import { PlatformInventory as Inventory, PlatformBlank as Blanks } from "@pythias/mongo";
...
export default async function InventoryPage(req) {
    ...
    const [res, allInventory, allBlanks] = await Promise.all([
        getInv({ Blanks, Inventory, term, page, orgId }),
        Inventory.find({ orgId, quantity: { $gt: 0 } }).select("quantity style_code size_name").lean(),
        Blanks.find({ orgId }).select("code sizes").lean(),
    ]);
