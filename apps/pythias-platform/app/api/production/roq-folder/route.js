...
import { PlatformItem as Items, PlatformOrder as Order, Manifest as manifest } from "@pythias/mongo";
...
export async function POST(req = NextApiRequest){
    ...
    console.log(data)
    let item = await Items.findOne({pieceId: data.scan, orgId}).populate("blank")
    if(item) item.order = await Order.findOne({_id: item.order, orgId}).populate("items")
