...
import { PlatformOrder as Order, Manifest as manifest } from "@pythias/mongo";
...
export async function POST(req = NextApiRequest) {
    ...
    const order = await Order.findOne({ _id: data.order._id, orgId });
    if (order) {
    ...
    const orders = await Order.find({ ...SHIPPED_QUERY, orgId })
