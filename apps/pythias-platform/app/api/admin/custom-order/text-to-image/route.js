...
const FONT_MAP    = { "Sans Serif": "sans-serif", "Serif": "serif", "Monospace": "monospace", "Bold": "sans-serif", "Italic": "sans-serif", "Condensed": "Arial Narrow, sans-serif" };
const FONT_WEIGHT = { "Bold": "bold" };
const FONT_STYLE  = { "Italic": "italic" };

export async function POST(request) {
    try {
        const { text = "", fontFamily = "Sans Serif", color = "#000000", bgColor } = await request.json();
  ...
  <text x="${width / 2}" y="${height / 2}" dominant-baseline="central" text-anchor="middle"
    font-family="${family}" font-size="${fSize}" font-weight="${weight}" font-style="${style}" fill="${color}">${esc(text)}</text>
</svg>`;

        const pngBuf = await sharp(Buffer.from(svg)).ensureAlpha().png().toBuffer();
        const key    = `text-images/${Date.now()}-${Math.random().toString(36).slice(2)}.png`;
        await s3.send(new PutObjectCommand({ Bucket: BUCKET, Key: key, Body: pngBuf, ACL: "public-read", ContentType: "image/png" }));
