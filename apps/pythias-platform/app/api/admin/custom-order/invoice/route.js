...
import { PlatformOrder as Order, PlatformItem as Item, Organization } from "@pythias/mongo";
...
async function buildPDF(order, items, brandName) {
    ...
    const customerLines = [addr.address2 || "", addr.name || "", order.customerEmail || "", addr.phone || "",
        [addr.address1, addr.city, addr.state, addr.zip].filter(Boolean).join(", ")].filter(Boolean);
    ...
    for (const item of items) {
        ...
        if (!lineMap.has(key)) lineMap.set(key, { styleCode: item.styleCode, colorName: item.colorName, sizeName: item.sizeName, price: item.price || 0, qty: 0, design: item.design || {} });
    ...
    y += 6;
    const shipping = order.shippingCost || 0, taxRate = order.taxRate || 0;
    const tax = subtotal * taxRate, total = subtotal + shipping + tax;
...
export async function GET(request) {
    ...
    const [order, org] = await Promise.all([
        Order.findOne({ _id: orderId, orgId, marketplace: "custom order" }).lean(),
        Organization.findById(orgId).select("name").lean(),
    ]);
    if (!order) return NextResponse.json({ error: "Order not found" }, { status: 404 });
    const items = await Item.find({ order: order._id, orgId }).lean();

    const buf      = await buildPDF(order, items, org?.name || "Pythias Platform");
...
export async function POST(request) {
    try {
        ...
        const brandName = org?.name || "Pythias Platform";
        const buf       = await buildPDF(order, items, brandName);
        const filename  = `invoice-${order.poNumber || orderId}.pdf`;
        const addr      = order.shippingAddress || {};
        ...
        await sendEmail({
            to: email,
            subject: `Invoice #${order.poNumber || orderId} — ${brandName}`,
            html: `
