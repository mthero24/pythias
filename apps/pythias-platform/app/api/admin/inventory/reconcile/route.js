...
import { PlatformInventory as Inventory, PlatformItem as Items } from "@pythias/mongo";
...
export async function POST(req) {
    ...
    const [inStockCounts, attachedCounts, allInvs] = await Promise.all([
        Items.aggregate([
            { $match: { orgId, stockStatus: "inStock", canceled: false, shipped: false, paid: true, "inventory.inventory": { $exists: true, $ne: null } } },
            { $group: { _id: "$inventory.inventory", count: { $sum: 1 } } },
        ]),
        Items.aggregate([
            { $match: { orgId, stockStatus: "attached", canceled: false, paid: true, "inventory.inventory": { $exists: true, $ne: null } } },
            { $group: { _id: "$inventory.inventory", count: { $sum: 1 } } },
        ]),
        Inventory.find({ orgId }, "_id").lean(),
    ]);
