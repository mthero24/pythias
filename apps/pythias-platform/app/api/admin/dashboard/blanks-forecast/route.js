...
import { PlatformItem as Items, PlatformBlank as Blank, PlatformInventory as Inventory, PlatformBlankForecastCache as BlankForecastCache } from "@pythias/mongo";
...
async function computeForecast(orgId) {
    ...
    const monthlyAgg = await Items.aggregate([
        { $match: { orgId, date: { $gte: since, $lte: until }, canceled: { $ne: true } } },
        { $group: {
            _id: {
                styleCode: "$styleCode",
                colorName: "$colorName",
    ...
    const [blanks, invDocs] = await Promise.all([
        Blank.find({ orgId, code: { $in: styleCodes } }).select("code sizes").lean(),
        Inventory.find({ orgId, style_code: { $in: styleCodes } })
...
export async function GET(req) {
    try {
        ...
        const cached = await BlankForecastCache.findOne({ appKey }).select("-_id -__v -appKey").lean();
...
export async function POST(req) {
    try {
        ...
        const appKey = orgId.toString();
        const result = await computeForecast(orgId);
        const computedAt = new Date();
        await BlankForecastCache.findOneAndUpdate(
            { appKey },
            { appKey, ...result, computedAt },
