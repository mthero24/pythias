...
import { PlatformOrder as Order, PlatformItem as Items, PlatformBlank as Blank, PlatformForecastCache as ForecastCache, PlatformDesign as Design, PlatformLicenseHolder as LicenseHolders, KlingInvoicePlatform } from "@pythias/mongo";
...
async function fetchHistorical(orgId, matchFilter) {
    ...
    const [rawDaily, dailyItemsAgg, licencedItemsAgg, klingInvoices] = await Promise.all([
        Order.aggregate([{$match:{...dateFilter,...orgFilter,...matchFilter}},{$group:{_id:{$dateToString:{format:"%Y-%m-%d",date:"$date"}},revenue:{$sum:{$ifNull:["$total",0]}},orders:{$sum:1}}},{$project:{_id:0,date:"$_id",revenue:1,orders:1}},{$sort:{date:1}}]),
        Items.aggregate([{$match:{...dateFilter,...orgFilter,canceled:{$ne:true}}},{$group:{_id:{date:{$dateToString:{format:"%Y-%m-%d",date:"$date"}},styleCode:"$styleCode",sizeName:"$sizeName"},qty:{$sum:1}}},{$project:{_id:0,date:"$_id.date",styleCode:"$_id.styleCode",sizeName:"$_id.sizeName",qty:1}}]),
        Items.aggregate([{$match:{...dateFilter,...orgFilter,designRef:{$ne:null},canceled:{$ne:true}}},{$group:{_id:{date:{$dateToString:{format:"%Y-%m-%d",date:"$date"}},designRef:{$toString:"$designRef"},styleCode:"$styleCode",sizeName:"$sizeName"},qty:{$sum:1},totalPrice:{$sum:{$ifNull:["$price",0]}}}},{$project:{_id:0,date:"$_id.date",designRef:"$_id.designRef",styleCode:"$_id.styleCode",sizeName:"$_id.sizeName",qty:1,totalPrice:1}}]),
        KlingInvoicePlatform.find({ orgId, year: { $gte: sinceYear, $lte: untilYear } }).lean(),
    ]);

    const styleCodes=[...new Set(dailyItemsAgg.map(r=>r.styleCode).filter(Boolean))];
    const styles=styleCodes.length?await Blank.find({orgId,code:{$in:styleCodes}}).select("code sizes").lean():[];
    const costMap={};
    ...
    if(licencedItemsAgg.length){
        ...
        const designs=await Design.find({orgId,_id:{$in:designIds},licenseHolder:{$ne:null}}).select("_id licenseHolder").lean();
        if(designs.length){
            const holderIds=[...new Set(designs.map(d=>d.licenseHolder).filter(Boolean).map(String))];
            const holders=await LicenseHolders.find({orgId,_id:{$in:holderIds}}).lean();
            ...
            const licStyles=licStyleCodes.length?await Blank.find({orgId,code:{$in:licStyleCodes}}).select("code sizes").lean():[];
            const retailMap={};
    ...
    return fillDays(since, until, rawDaily).map(d=>({...d,cogs:cogsByDate[d.date]||0,net:Math.max(0,d.revenue-(cogsByDate[d.date]||0)-(licenceFeeByDate[d.date]||0)-(klingCostByDate[d.date]||0))}));
}
...
export async function GET(req) {
    try {
        ...
        const appKey = orgId.toString();

        if (marketplace && marketplace !== "All") {
            const historical = await fetchHistorical(orgId, { canceled:{$ne:true}, refunded:{$ne:true}, marketplace });
        ...
        const cached = await ForecastCache.findOne({ appKey }).select("-_id -__v -appKey").lean();
