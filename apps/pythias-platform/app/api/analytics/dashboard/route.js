...
export async function GET(req) {
    ...
    if (!token?.permissions?.charts && token?.role !== "admin") {
    ...
    const [
        totalViews, humanViews, botViews,
        uniqueSessions, humanSessions,
        topPages, topSources, vitalsPerPage, trafficByDay, botReasons, recentSessions,
    ] = await Promise.all([
        ...
        PageView.aggregate([
            ...
            { $group: {
                _id: "$page",
                lcp: { $avg: "$vitals.lcp" }, cls: { $avg: "$vitals.cls" },
                ttfb: { $avg: "$vitals.ttfb" }, fcp: { $avg: "$vitals.fcp" },
                inp: { $avg: "$vitals.inp" }, loadTime: { $avg: "$vitals.loadTime" },
                sampleCount: { $sum: 1 },
            }},
            { $sort: { sampleCount: -1 } },
            { $limit: 15 },
        ...
        PageView.aggregate([
            { $match: matchHuman },
            { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$enteredAt" } }, views: { $sum: 1 } } },
            { $sort: { _id: 1 } },
        ]),
        PageView.aggregate([
            { $match: matchBot },
        ...
        Session.find({ startedAt: { $gte: since }, isBot: false }).sort({ startedAt: -1 }).limit(20).lean(),
    ]);
    ...
    return NextResponse.json({
        ...
        vitalsPerPage: vitalsPerPage.map(v => ({
            page: v._id,
            lcp: Math.round(v.lcp ?? 0), cls: Math.round((v.cls ?? 0) * 1000) / 1000,
            ttfb: Math.round(v.ttfb ?? 0), fcp: Math.round(v.fcp ?? 0),
            inp: Math.round(v.inp ?? 0), loadTime: Math.round(v.loadTime ?? 0),
        ...
        recentSessions: recentSessions.map(s => ({
            sessionId: s.sessionId, startedAt: s.startedAt, pages: s.pages,
            totalTime: s.totalTime, source: s.source,
            entryPage: s.entryPage, exitPage: s.exitPage, referrer: s.referrer,
        })),
    });
}
