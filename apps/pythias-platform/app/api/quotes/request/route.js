...
// PUBLIC (see middleware allowlist). The customer "Design & Send In" studio submits here. The shop
// is identified by orgSlug (public). Creates a `requested` quote with ZERO prices — the shop prices
// it, then the existing send → /quote/[token] → approve+pay → convert flow takes over. No pricing
// is ever exposed to the customer at this stage.
...
export async function POST(request) {
    try {
        ...
        const org  = data.orgSlug ? await Organization.findOne({ slug: data.orgSlug }).select("_id name billingEmail").lean() : null;
        if (!org) return NextResponse.json({ error: "Unknown shop" }, { status: 404 });
