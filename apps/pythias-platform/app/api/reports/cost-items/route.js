...
export async function GET(req) {
    try {
        ...
        const basePipeline = [
            { $match: filter },
            { $lookup: { from: PlatformOrder.collection.name, localField: "order", foreignField: "_id", as: "_ord" } },
        ...
        const [rawItems, countAgg] = await Promise.all([
            PlatformItem.aggregate([
                ...
                { $limit: pageSize },
                { $project: { date: 1, styleCode: 1, sizeName: 1, colorName: 1, price: 1, orderId: 1, poNumber: 1, marketplace: 1 } },
            ]),
            csvMode ? Promise.resolve([]) : PlatformItem.aggregate([...basePipeline, { $count: "total" }]),
        ]);
        const total = csvMode ? 0 : (countAgg[0]?.total ?? 0);

        const styleCodes = [...new Set(rawItems.map(i => i.styleCode).filter(Boolean))];
        const blanks = styleCodes.length ? await PlatformBlank.find({ orgId, code: { $in: styleCodes } }).select("code sizes").lean() : [];
        const costMap = {};
        for (const b of blanks) { costMap[b.code] = {}; for (const sz of b.sizes ?? []) costMap[b.code][sz.name] = sz.wholesaleCost ?? 0; }
        const items = rawItems.map(i => ({ ...i, wholesaleCost: costMap[i.styleCode]?.[i.sizeName] ?? 0 }));

        if (csvMode) {
            const esc = (v) => { const s = String(v ?? ""); return s.includes(",") || s.includes('"') || s.includes("\n") ? `"${s.replace(/"/g, '""')}"` : s; };
            const headers = ["Date", "Channel", "Blank Code", "Size", "Color", "Price Sold", "Blank COGS", "Est. MP Fees", "Net"];
            const rows = items.map(i => {
                const mpFee = (i.price || 0) * (MP_RATES[(i.marketplace || "").toLowerCase()] ?? 0);
                const net = (i.price || 0) - (i.wholesaleCost || 0) - mpFee;
                return [i.date ? new Date(i.date).toLocaleDateString() : "", i.marketplace || "", i.styleCode || "", i.sizeName || "", i.colorName || "", (i.price ?? 0).toFixed(2), (i.wholesaleCost ?? 0).toFixed(2), mpFee.toFixed(2), net.toFixed(2)];
            });
        ...
        console.error("[reports/cost-items]", e);
