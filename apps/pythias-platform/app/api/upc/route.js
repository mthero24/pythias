...
import {SkuToUpc as UpcToSku, Organization} from "@pythias/mongo";
...
async function gs1Configured(req) {
    const token = await getToken({ req });
    if (!token?.orgId) return false;
    const org = await Organization.findById(token.orgId).select("settings.gs1").lean();
    return !!(org?.settings?.gs1?.apiKey);
}

export async function POST(req=NextApiRequest){
    if (!await gs1Configured(req)) return NextResponse.json({ error: false, upcs: [] });
