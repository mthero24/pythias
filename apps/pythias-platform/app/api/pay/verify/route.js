...
import { PlatformOrder as Order, PlatformItem as Item, PlatformInventory as Inventory, Organization } from "@pythias/mongo";
import { updateInventory, recomputeStockStatus } from "@/functions/pullOrders";
import Stripe from "stripe";

// POST /api/pay/verify { t, sessionId } — PUBLIC (see middleware). Confirms the Stripe Checkout
// session is paid on the seller's connected account, then marks the order paid + awaiting_shipment.
// Safe to call repeatedly / by anyone: it only acts when Stripe reports payment_status === "paid"
// for a session whose metadata.orderId matches the order found by the (unguessable) invoice token.
export async function POST(request) {
    try {
        ...
        const org       = await Organization.findById(order.orgId).select("name storefrontConnect").lean();
        ...
        await order.save();
        await Item.updateMany({ order: order._id, orgId: order.orgId }, { $set: { paid: true, status: "awaiting_shipment" } });

        // Slot each item into an inventory record so the recompute can place it correctly.
        const items = await Item.find({ order: order._id, orgId: order.orgId });
        for (const item of items) {
            if (item.inventory?.inventory) continue;
            const inv = await Inventory.findOne({ orgId: order.orgId, color_name: item.colorName, size_name: item.sizeName, style_code: item.styleCode });
