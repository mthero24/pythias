...
import { PlatformQuote as Quote, PlatformOrder as Order, PlatformItem as Item, Organization } from "@pythias/mongo";
import { generatePieceID } from "@pythias/integrations";
import Stripe from "stripe";

// PUBLIC (see middleware) — the customer's quote review + approve/pay page calls this.
// GET → display data. POST { action:"checkout" } → Stripe pay link. POST { action:"verify", sessionId }
// → confirm payment and convert the quote into a paid production order. Pay-then-convert, so an
// abandoned checkout never leaves an orphan order. Direct charge on the seller's connected account
// (2% application fee → Pythias), mirroring the invoice flow.

// Build a paid production Order + Items from an approved quote.
async function convertQuoteToOrder(quote, orgId) {
...
export async function GET(request, { params }) {
    ...
    const org = await Organization.findById(quote.orgId).select("name").lean();
    return NextResponse.json({
        brandName: org?.name || "Pythias",
        quote: {
...
export async function POST(request, { params }) {
    try {
        ...
        const org       = await Organization.findById(quote.orgId).select("name storefrontConnect").lean();
        ...
        if (action === "verify") {
            ...
            const order = await convertQuoteToOrder(quote, quote.orgId);
