...
// Returns a vitals ref object and a cleanup fn. Runs once per app mount.
function useWebVitals(vitalsRef) {
    useEffect(() => {
        const observers = [];

        // LCP — updates as better candidates arrive
        try {
        ...
        // TTFB, FCP, total load — from Navigation Timing
...
export default function AnalyticsTracker() {
    ...
    useEffect(() => {
        ...
        // Detect any real user interaction
        ...
        // UTM params (read synchronously, no hook needed)
        ...
        // Fire on: SPA navigation (cleanup), tab hide, browser close
        const onVisibility = () => { if (document.visibilityState === "hidden") sendLeave(); };
        window.addEventListener("beforeunload",    sendLeave);
        document.addEventListener("visibilitychange", onVisibility);

        return () => {
            sendLeave(); // SPA navigation to next page
