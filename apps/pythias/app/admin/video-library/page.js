...
const TABS = [
    ...
    { label: "Walkthroughs", type: "walkthrough" },
...
const emptyForms = {
    ...
    walkthrough: { title: "", description: "", product: "fulfillment-cloud", targetPage: "/fulfillment-cloud", order: 0 },
...
export default function AdminVideoLibraryPage() {
    ...
    async function save() {
        ...
        try {
            ...
            const folderMap = {
                ...
                demo:        "demo-videos",
                "page-video":"page-videos",
            };

            if (videoFile) {
                setProgress("Uploading video…");
                videoUrl = await uploadRaw(videoFile, folderMap[currentType] || "videos");
            }
            if (thumbFile) {
            ...
            const res     = await fetch("/api/admin/tutorials", { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
            const data    = await res.json();

            if (!res.ok) { setSaveError(data.error || `Save failed (${res.status})`); return; }
    ...
    function renderTableHead() {
        ...
        if (currentType === "walkthrough") return <><TableCell sx={{ fontWeight: 700 }}>Product</TableCell><TableCell sx={{ fontWeight: 700 }}>Page</TableCell><TableCell sx={{ fontWeight: 700 }}>Order</TableCell></>;
    ...
    function renderTableCells(item) {
        ...
        if (currentType === "walkthrough") return <><TableCell><Chip label={item.product || "FC"} size="small" variant="outlined" /></TableCell><TableCell sx={{ fontSize: 13 }}>{item.targetPage}</TableCell><TableCell sx={{ fontSize: 13, color: "text.secondary" }}>{item.order}</TableCell></>;
    ...
    function canSave() {
        ...
        if (currentType === "walkthrough") return !!form.title && !!form.product;
            ...
            <Dialog open={open} onClose={() => !uploading && setOpen(false)} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
                        ...
                        <Box>
                            <Typography variant="body2" fontWeight={600} sx={{ mb: 0.5 }}>
                                Thumbnail image {editing ? "(leave blank to keep existing)" : "(optional)"}
                            </Typography>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
