...
import {
    ...
    Alert, Accordion, AccordionSummary, AccordionDetails, InputAdornment, IconButton,
...
function PrinterRow({ printer, onChange, onDelete }) {
    return (
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems="flex-start">
            ...
            <TextField label="Port" value={printer.port ?? "9100"} onChange={e => onChange("port", e.target.value)} size="small" sx={{ width: 80 }} />
...
function ScaleRow({ scale, onChange, onDelete }) {
    return (
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems="flex-start">
            ...
            <TextField label="Port" value={scale.port ?? "8080"} onChange={e => onChange("port", e.target.value)} size="small" sx={{ width: 80 }} />
...
export default function ShippingSettingsPage() {
    ...
    function deepMerge(base, override) {
        ...
        for (const k of Object.keys(override ?? {})) {
            ...
            else if (override[k] && typeof override[k] === "object") { out[k] = deepMerge(base[k] ?? {}, override[k]); }
    ...
    function setArrayItem(key, idx, field, value) {
        setCreds(c => { const arr = [...(c[key] ?? [])]; arr[idx] = { ...arr[idx], [field]: value }; return { ...c, [key]: arr }; });
    }
    ...
    async function save(e) {
        ...
        setMsg(d.ok ? { type: "success", text: "Shipping settings saved" } : { type: "error", text: d.error ?? "Save failed" });
        setSaving(false);
    }
            ...
            <Container maxWidth="md" sx={{ py: 4 }}>
                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
                    ...
                    <Stack direction="row" spacing={1}>
                        <Button href="/admin/settings/shipping/guide" variant="outlined" size="small">Setup Guide →</Button>
                        <Button href="/admin/settings/integrations" variant="outlined" size="small">Marketplace Integrations →</Button>
                    </Stack>
                </Stack>
                ...
                <form onSubmit={save}>
                    <Stack spacing={2}>
                        ...
                        <Accordion defaultExpanded>
                            ...
                            <AccordionDetails>
                                <Stack spacing={2}>
                                    <Typography variant="caption" color="text.secondary">IP and API key for your on-site Pythias Internal Server (DTF files, label printing, floor stations).</Typography>
                                    <TextField label="Internal Server IP" value={creds.localIP} onChange={set("localIP")} fullWidth size="small" placeholder="0.0.0.0:3005" helperText="e.g. 192.168.1.50:3005" />
                                    <MaskedField label="Internal Server API Key" value={creds.localKey} onChange={set("localKey")} helperText="Bearer token from internal server settings" />
                                </Stack>
                            </AccordionDetails>
                        </Accordion>
                        ...
                        <Accordion defaultExpanded>
                            ...
                            <AccordionDetails>
                                <Stack spacing={2}>
                                    <Typography variant="caption" color="text.secondary">Origin address on all outbound shipments and return address on labels.</Typography>
                                    <Stack direction="row" spacing={1}>
                                    ...
                                    <TextField label="Address line 2" value={creds.businessAddress.address2} onChange={set("businessAddress.address2")} fullWidth size="small" />
                                    <Stack direction="row" spacing={1}>
                        ...
                        <Accordion>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography fontWeight={700}>ShipStation</Typography></AccordionSummary>
                            <AccordionDetails>
                                <Stack spacing={2}>
                                    ...
                                    <MaskedField label="V2 Bearer Token" value={creds.shipstation.v2Key} onChange={set("shipstation.v2Key")} helperText="Account Settings → API Keys → V2 key" />
                                </Stack>
                            </AccordionDetails>
                        </Accordion>

                        <Accordion>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography fontWeight={700}>USPS</Typography></AccordionSummary>
                            <AccordionDetails>
                                <Stack spacing={2}>
                        ...
                        <Accordion>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography fontWeight={700}>UPS</Typography></AccordionSummary>
                            <AccordionDetails>
                                <Stack spacing={2}>
                        ...
                        <Accordion>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography fontWeight={700}>Endicia / Stamps.com</Typography></AccordionSummary>
                            <AccordionDetails>
                                <Stack spacing={2}>
                        ...
                        <Accordion>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography fontWeight={700}>FedEx</Typography></AccordionSummary>
                            <AccordionDetails>
                                <Stack spacing={2}>
                        ...
                        <Accordion>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography fontWeight={700}>Label Printers</Typography></AccordionSummary>
                            <AccordionDetails>
                                <Stack spacing={3}>
                                    <Typography variant="caption" color="text.secondary">
                                        Compatible: Zebra ZD421/ZD621 (ZPL, direct IP), Rollo (ZPL/PDF), DYMO LabelWriter 4XL (PDF, USB bridge), Munbyn ITPP941 (ZPL/PDF).
                                        ZPL for direct network printing; PDF for USB-only printers bridged via the internal server.
                                    </Typography>
                                    <Box>
                                        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
                                            <Typography variant="subtitle2" fontWeight={700}>Shipping Label Printers</Typography>
                                            <Button size="small" startIcon={<AddIcon />} onClick={() => addArrayItem("shippingLabelPrinters", { name: "", ipAddress: "", port: "9100", format: "ZPL" })}>Add</Button>
                                        </Stack>
                                        <Stack spacing={1}>
                                            {(creds.shippingLabelPrinters ?? []).map((p, i) => (
                                                <PrinterRow key={i} printer={p} onChange={(f, v) => setArrayItem("shippingLabelPrinters", i, f, v)} onDelete={() => removeArrayItem("shippingLabelPrinters", i)} />
                                            ))}
                                            {!creds.shippingLabelPrinters?.length && <Typography variant="body2" color="text.secondary">None added.</Typography>}
                                        </Stack>
                                    </Box>
                                    <Divider />
                                    <Box>
                                        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
                                            ...
                                            <Button size="small" startIcon={<AddIcon />} onClick={() => addArrayItem("productionLabelPrinters", { name: "", ipAddress: "", port: "9100", format: "ZPL" })}>Add</Button>
                                        </Stack>
                                        <Stack spacing={1}>
                                            {(creds.productionLabelPrinters ?? []).map((p, i) => (
                                                <PrinterRow key={i} printer={p} onChange={(f, v) => setArrayItem("productionLabelPrinters", i, f, v)} onDelete={() => removeArrayItem("productionLabelPrinters", i)} />
                                            ))}
                                            {!creds.productionLabelPrinters?.length && <Typography variant="body2" color="text.secondary">None added.</Typography>}
                                        </Stack>
                                    </Box>
                                </Stack>
                            </AccordionDetails>
                        ...
                        <Accordion>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography fontWeight={700}>Shipping Scales</Typography></AccordionSummary>
                            <AccordionDetails>
                                <Stack spacing={2}>
                                    <Typography variant="caption" color="text.secondary">
                                        Compatible (USB-to-network via internal server): Stamps.com 5 lb Digital Scale, Dymo M25 (25 lb), Fairbanks SCB-R9000, Adam Equipment CBD 4 Compact.
                                    </Typography>
                                    <Stack direction="row" justifyContent="flex-end">
                                        <Button size="small" startIcon={<AddIcon />} onClick={() => addArrayItem("scales", { name: "", ipAddress: "", port: "8080", model: "" })}>Add Scale</Button>
                                    </Stack>
                                    <Stack spacing={1}>
                                        {(creds.scales ?? []).map((s, i) => (
                                            <ScaleRow key={i} scale={s} onChange={(f, v) => setArrayItem("scales", i, f, v)} onDelete={() => removeArrayItem("scales", i)} />
                                        ))}
                                        {!creds.scales?.length && <Typography variant="body2" color="text.secondary">No scales configured.</Typography>}
                                    </Stack>
                                </Stack>
                            </AccordionDetails>
                        </Accordion>
