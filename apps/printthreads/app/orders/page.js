...
export default async function OrdersPage(req){
    ...
    let orders = await Order.find({ "items.0": {$exists: true} }).sort({date: -1}).populate("items").select("poNumber marketplace items status date total").skip((page * 200) - 200).limit(200)
    let count = await Order.find({ "items.0": {$exists: true} }).countDocuments()
