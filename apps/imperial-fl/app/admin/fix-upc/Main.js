"use client";
import {Box, Card, Grid2, Typography, Button, Modal, TextField, ButtonGroup} from "@mui/material";
...
export function Main({s, count}){
    ...
    const searchSku = async () => {
        let res = await axios.get(`/api/upc?sku=${search}`)
        setSku(res.data.upc)
        setCount(res.data.count)
        setSearch("")
    }
    return (
        <Box sx={{background: "#e2e2e2", padding: "3%"}}>
            <ButtonGroup>
                <TextField fullWidth label="search" value={search} onChange={()=> {setSearch(event.target.value)}} onKeyDown={()=>{
