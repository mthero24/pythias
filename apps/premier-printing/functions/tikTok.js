...
export async function shipOrderTikTok({ order, items, trackingNumber, provider }) {
    ...
    // Resolve the TikTok shipping provider id (scoped to the delivery option) from the carrier we shipped with.
    ...
    // Prefer the line item ids from TikTok's order detail; fall back to ids captured on our items.
      ...
      try {
        ...
        if (!order) {
            ...
            for (let i of (o.line_items || [])) {
                // TikTok line items expose `seller_sku` / `sku_name` / `id` — they have NO `sku`,
                // `name`, or `orderItemId` fields. The parser + item builders below all read i.sku /
                // i.name, so without this mapping every blank/color/size/design lookup gets undefined
                // and the item lands completely blank. Normalize once here.
