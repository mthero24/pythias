...
export async function POST(req) {
    ...
    const plan = await ServicePlanPremier.create({ appName, description: description ?? "", monthlyPrice });
...
export async function PUT(req) {
    ...
    const plan = await ServicePlanPremier.findByIdAndUpdate(_id, update, { new: true });
    return NextResponse.json({ plan });
}
...
export async function DELETE(req) {
    ...
    if (!_id) return NextResponse.json({ error: "Missing _id" }, { status: 400 });
    await ServicePlanPremier.findByIdAndDelete(_id);
