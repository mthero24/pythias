...
export async function GET(req) {
    ...
    const range = req.nextUrl.searchParams.get("range") || "day"; // hour | day | week | month
    ...
    else since = new Date(now - 24 * 60 * 60 * 1000); // day
    ...
    try {
        ...
        // Time buckets: group by hour for granular view
        const bucketSize = range === "hour" ? 5 : range === "week" ? 360 : range === "month" ? 1440 : 60; // minutes
        ...
        // Reshape byUser into { userName -> { action -> count } }
        const users = {};
