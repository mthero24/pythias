...
export async function POST(req=NextApiRequest){
    ...
    //console.log(data)
    console.log(data)
    let order = new InventoryOrders({vendor: data.order.company, poNumber: data.order.poNumber, orderType: data.orderType === "outOfStock" ? "outOfStock" : "inventory", dateOrdered: new Date(data.order.dateOrdered + "T12:00:00"), dateExpected: data.order.dateExpected? new Date(data.order.dateExpected + "T12:00:00"): null, locations: [], items: data.items})
    let locations = []
