...
export async function PUT(req = NextApiRequest) {
    ...
    if(order){
        ...
        await order.save()
        logChange({ entityType: "inventory_order", entityId: order._id, entityName: order.poNumber || "", action: "update", userName, email, provider: "premierPrinting" });
...
export async function POST(req = NextApiRequest) {
    ...
    if (order) {
        ...
        await order.save()
        logChange({ entityType: "inventory_order", entityId: order._id, entityName: order.poNumber || "", action: "delete_item", userName, email, provider: "premierPrinting" });
        return NextResponse.json({ msg: "Order updated successfully", error: false })
    } else {
        return NextResponse.json({ error: "Order not found" })
    }
