...
export async function POST(request) {
    try {
        ...
        const [order, org] = await Promise.all([
            Order.findOne({ _id: orderId }),   // custom orders + customer-service orders
            Organization.findOne({ slug: process.env.PREMIER_ORG_SLUG || "premier-printing" }).select("name storefrontConnect").lean(),
        ]);
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            metadata: { orderId: String(order._id), invoiceToken },
