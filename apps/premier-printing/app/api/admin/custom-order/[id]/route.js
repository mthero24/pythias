...
import { Order, Item, Inventory } from "@pythias/mongo";
import { getToken } from "next-auth/jwt";
import { logActivity, userFromToken } from "@pythias/backend/server";
import { updateInventory, recomputeStockStatus } from "@/functions/pullOrders";

export async function GET(request, { params }) {
    const order = await Order.findOne({ _id: params.id, marketplace: "custom order" }).populate("items").lean();
...
export async function PATCH(request, { params }) {
    ...
    // Any order can be marked paid here — custom orders and "customer service" orders from the
    // new-order modal both flow through this PATCH.
    ...
    if (data.paid === true && !order.paid) {
        ...
        // Mark all items paid and move them to awaiting_shipment
        ...
        for (const item of items) {
            if (item.inventory?.inventory) continue; // already assigned
    ...
    await order.save();
    logActivity({ action: "custom_order_update", entity: "order", entityId: order._id, entityName: order.poNumber || "", userName, email, provider: "premierPrinting" });
...
export async function DELETE(request, { params }) {
    const token = await getToken({ req: request });
    const { userName, email } = userFromToken(token);
    const order = await Order.findOne({ _id: params.id, marketplace: "custom order" });
    if (!order) return NextResponse.json({ error: "Not found" }, { status: 404 });
    await Item.deleteMany({ order: order._id });
    await order.deleteOne();
    logActivity({ action: "custom_order_delete", entity: "order", entityId: params.id, entityName: "", userName, email, provider: "premierPrinting" });
