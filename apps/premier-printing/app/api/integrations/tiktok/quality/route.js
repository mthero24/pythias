...
import { getProductQualityTikTok, getAccessTokenFromRefreshToken } from "@pythias/integrations";
...
export async function GET(req) {
    ...
    let res = await getProductQualityTikTok(productId, credentials, shopCipher);
    if (res.error && res.msg === "refresh") {
        credentials = await refreshCredentials(credentials);
        res = await getProductQualityTikTok(productId, credentials, shopCipher);
    }
    if (res.error) return NextResponse.json({ error: res.msg }, { status: 500 });

    return NextResponse.json({ quality_tier: res.quality_tier, issues: res.issues ?? [] });
}
