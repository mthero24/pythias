...
// envelope so GTX prints the cropped art at its real size/spot (horizoffset from left, vertoffset from
// top). Pre-made designs (no placement) keep the full envelope. Compute offsets from the ORIGINAL dims.
...
async function getItemForGTX(pieceId) {
    return Items.findOne({ pieceId: pieceId.toUpperCase().trim() })
...
export async function GET(req) {
    ...
    const [onPrinter, printerQue] = await Promise.all([
        Items.findOne({ onPrinter: true, "printerQue.printer": printer })
            .populate("blank", "code envelopes")
            .populate("order", "poNumber items notes")
            .populate("color", "category name color_type")
            .lean(),
        Items.find({ onPrinter: false, "printerQue.printer": printer })
            .populate("blank", "code envelopes")
...
export async function POST(req) {
    ...
    try {
        ...
        if (action === "scan") {
            ...
            for (const id of ids) {
                try {
                    const item = await Items.findOne({ pieceId: id.toUpperCase().trim() })
            ...
            const [onPrinter, printerQue] = await Promise.all([
                Items.findOne({ onPrinter: true, "printerQue.printer": printer }).populate("blank", "code envelopes").populate("order", "poNumber items notes").populate("color", "category name color_type").lean(),
                Items.find({ onPrinter: false, "printerQue.printer": printer }).populate("blank", "code envelopes").populate("order", "poNumber items").populate("color", "category name color_type").sort({ "printerQue.scan": 1 }).lean(),
            ]);
        ...
        if (action === "send-to-printer") {
            const item = await Items.findOne({ pieceId: pieceID.toUpperCase().trim() }).populate("blank", "code envelopes").populate("order", "poNumber items notes").populate("color", "category name color_type");
            if (!item) return NextResponse.json({ error: true, msg: "Item not found" });

            const onPrinter = await Items.findOne({ onPrinter: true, "printerQue.printer": printer }).populate("blank", "code envelopes").populate("order", "poNumber items").populate("color", "category name color_type");
            const job = buildGTXJob(item);
        ...
        if (action === "repull") {
            const item = await Items.findOne({ pieceId: pieceID.toUpperCase().trim() }).populate("blank", "code envelopes").populate("order", "poNumber items").populate("color", "category name color_type");
            if (!item) return NextResponse.json({ error: true, msg: "Item not found" });
            const job = buildGTXJob(item);
            await axios.post(`http://${sc.localIP}/api/gtx/send`, { que: job, createOnly: false }, { headers: internalHeaders(sc.localKey) });
        ...
        if (action === "clear-all") {
            const queued = await Items.find({ "printerQue.printer": printer });
