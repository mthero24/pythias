...
export async function POST(request, { params }) {
    try {
        ...
        const org = await Organization.findOne({ slug: process.env.PREMIER_ORG_SLUG || "premier-printing" }).select("name").lean();
        ...
        await sendEmail({
            to: email,
            subject: `Your quote from ${org?.name || "Premier Printing"} — $${due.toFixed(2)}`,
            html: quoteEmailHtml({ brandName: org?.name || "Premier Printing", quote, url, due }),
        });
