...
// Single debug hub for ALL integrations, both surfaces (orders + listing).
// Read-only diagnose by default; explicit buttons to run the real order pull.
// Hidden behind ?debug=1 on the page. Safe to keep in place permanently.
const MARKETPLACES = ["walmart", "faire", "amazon", "ebay", "acenda", "etsy", "shopify", "tiktok"];

export default function IntegrationDebugHub() {
    const [out, setOut] = useState(null);
    const [busy, setBusy] = useState(false);
    const [label, setLabel] = useState("");
    const [tp, setTp] = useState({ marketplace: "walmart", connectionId: "", productId: "" });
    ...
    async function testPush() {
        if (!tp.productId || (tp.marketplace !== "tiktok" && !tp.connectionId)) {
            setOut({ error: true, msg: "productId required (and connectionId for non-TikTok)" });
            return;
        }
        setBusy(true);
    ...
    return (
        <div style={{ margin: 24, padding: 20, border: "1px dashed #D3A73D", borderRadius: 12 }}>
            ...
            <p style={{ margin: "0 0 14px", color: "#64748b", fontSize: 13 }}>
                Read-only diagnostics for every integration. Counts/errors per source — orders and listing.
            </p>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                ...
                <button style={btn} disabled={busy} onClick={() => call("?pull=1", "Running full pull")}>Run Full Order Pull</button>
            </div>
            ...
            <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid #1e293b" }}>
                ...
                <p style={{ margin: "0 0 10px", color: "#64748b", fontSize: 12 }}>
                    ⚠ Real push — creates/updates a live listing. Returns the marketplace&apos;s accept/reject + warnings.
                    Grab <code>connectionId</code> from the diagnose output above.
                </p>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                    <select style={input} value={tp.marketplace} onChange={e => setTp({ ...tp, marketplace: e.target.value })}>
                        {MARKETPLACES.map(m => <option key={m} value={m}>{m}</option>)}
                    </select>
                    <input style={{ ...input, width: 230 }} placeholder="connectionId (not needed for tiktok)" value={tp.connectionId} onChange={e => setTp({ ...tp, connectionId: e.target.value })} />
                    <input style={{ ...input, width: 230 }} placeholder="productId" value={tp.productId} onChange={e => setTp({ ...tp, productId: e.target.value })} />
