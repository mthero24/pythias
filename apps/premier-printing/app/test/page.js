...
export default async function Test(){
    ...
    let designs = await Design.find({published: true}).populate("brands b2m blanks.blank blanks.colors blanks.defaultColor").sort({'_id': -1}).limit(750)
    let brands = {}
    let i = 0
    ...
    const createTargetVariant = ({p,item, v, price, bImages})=>{
        return {
            ...
            gtin: v.gtin,
            "description": `${p.design.description} - ${v.size.name} - ${v.color.name} ${p.blank.blank.description}`,
            brand: p.brand,
            "sku": v.sku,
