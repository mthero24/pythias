...
const NAV_SECTIONS = [
  ...
  {
    // Storefront is a seller product; in the fulfiller app we only surface the explainer for now.
    label: "Storefront",
    items: [
      { label: "Learn about Storefront", href: "/admin/store/welcome", icon: <StorefrontIcon fontSize="small" />, showCSV: false },
    ],
  },
  {
    label: "Orders",
...
export default function ButtonAppBar() {
  ...
  if (pathname === "/login" || pathname?.startsWith("/pay") || pathname?.startsWith("/quote/") || pathname?.startsWith("/design")) return null;
