...
export async function getShippingCreds() {
    ...
    return {
        ...
        credentialsFedExNew: {
            accountNumber: g(m, "fedex.accountNumber", process.env.AccountFedExTest),
            key:           g(m, "fedex.clientId",      process.env.ApiKeyTestFedEx),
            secret:        g(m, "fedex.clientSecret",  process.env.SecretKeyFedExTest),
        },
        credentialsFedEx: {
        ...
        stations: (() => {
            try {
                ...
                return (prod.shippingStations ?? []).map(s =>
                    typeof s === "string" ? { name: s, hasScale: true, format: "PDF" } : { format: "PDF", ...s }
                );
            } catch { return []; }
        })(),
        labelPrinters: (() => {
