...
export async function buyLabel({address, poNumber, weight, selectedShipping, dimensions, businessAddress, providers, enSettings,
    ...
    credentialsFedEx,
    credentialsFedExNew,
    credentialsUPS, dpi, ignoreBadAddress, imageFormat}){
    if(selectedShipping.provider == "usps"){
        if(providers.includes("endicia")){
            let res = await buyShippingLabelEn({address, poNumber, weight, selectedShipping, dimensions, businessAddress, enSettings, dpi,imageFormat})
            return res
        }
        else if(providers.includes("usps")){
            let res = await purchaseLabel({address, poNumber, weight, selectedShipping, dimensions, businessAddress, credentials, dpi, ignoreBadAddress})
            return res
