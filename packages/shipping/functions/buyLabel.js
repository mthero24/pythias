...
export async function buyLabel({address, poNumber, weight, selectedShipping, dimensions, businessAddress, providers, enSettings,
    ...
    if(selectedShipping.provider == "usps"){
        ...
        if(providers.includes("fedex")){
            ...
            let res = await purchaseLabelFedEx({ address, poNumber, weight: weight / 16, selectedShipping, dimensions, businessAddress, credentials: credentialsFedExNew, saturdayDelivery: items && items[0]?.saterdayDelivery ? items[0].saterdayDelivery : false, imageFormat, dpi})
            return res
