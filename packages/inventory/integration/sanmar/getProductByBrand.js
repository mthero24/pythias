...
export async function getProductInfoByBrand(brand) {
    let xml = `
    ...
    let error = data["S:Envelope"]["S:Body"][0]["ns2:getProductInfoByBrandResponse"][0]["return"][0]["errorOccured"][0] == "false" ? false : true
    let jsonProducts = []
    if (!error) {
        let products = data["S:Envelope"]["S:Body"][0]["ns2:getProductInfoByBrandResponse"][0]["return"][0]["listResponse"]
