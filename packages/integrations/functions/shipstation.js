...
export async function getOrders({auth, id}){
    ...
    if (id) {
        const res = await axios.get(`https://ssapi.shipstation.com/orders?orderNumber=${id}&pageSize=500`, headers).catch(e => { console.log(e.response.data) })
    ...
    let res = await axios.get(`https://ssapi.shipstation.com/orders?createDateStart=${(lastDate.getMonth() + 1).toString().length == 2 ? lastDate.getMonth() + 1 : `0${lastDate.getMonth() + 1}`}/${lastDate.getDate().toString().length == 2 ? lastDate.getDate() : `0${lastDate.getDate()}`}/${lastDate.getFullYear()}&orderStatus=awaiting_shipment&page=${1}&pageSize=500`, headers).catch(e=>{console.log(e.response.data)})
    ...
    for(let i = 2; i <= parseInt(res?.data?.pages); i++){
        ...
        let res = await axios.get(`https://ssapi.shipstation.com/orders?createDateStart=${(lastDate.getMonth() + 1).toString().length == 2 ? lastDate.getMonth() + 1 : `0${lastDate.getMonth() + 1}`}/${lastDate.getDate().toString().length == 2 ? lastDate.getDate() : `0${lastDate.getDate()}`}/${lastDate.getFullYear()}&orderStatus=awaiting_shipment&page=${i}&pageSize=500`, headers).catch(e=>{console.log(e.response.data)})
        orders = orders.concat(res.data.orders)
    }
    let res2 = await axios.get(`https://ssapi.shipstation.com/orders?createDateStart=${(lastDate.getMonth() + 1).toString().length == 2 ? lastDate.getMonth() + 1 : `0${lastDate.getMonth() + 1}`}/${lastDate.getDate().toString().length == 2 ? lastDate.getDate() : `0${lastDate.getDate()}`}/${lastDate.getFullYear()}&page=${1}&orderStatus=cancelled&pageSize=500`, headers).catch(e => { console.log(e.response.data) })
    if(res2.data && res2.data.orders){
        orders = orders.concat(res2.data.orders)
    }
    for (let i = 2; i <= parseInt(res2?.data?.pages); i++) {
        console.log(orders.length, i)
        let res = await axios.get(`https://ssapi.shipstation.com/orders?createDateStart=${(lastDate.getMonth() + 1).toString().length == 2 ? lastDate.getMonth() + 1 : `0${lastDate.getMonth() + 1}`}/${lastDate.getDate().toString().length == 2 ? lastDate.getDate() : `0${lastDate.getDate()}`}/${lastDate.getFullYear()}&page=${i}&orderStatus=cancelled&pageSize=500`, headers).catch(e => { console.log(e.response.data) })
        orders = orders.concat(res.data.orders)
    }
