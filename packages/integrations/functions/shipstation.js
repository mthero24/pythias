...
export async function getOrders({auth, id}){
    ...
    let res = await axios.get(`https://ssapi.shipstation.com/orders?createDateStart=${(lastDate.getMonth() + 1).toString().length == 2 ? lastDate.getMonth() + 1 : `0${lastDate.getMonth() + 1}`}/${lastDate.getDate().toString().length == 2 ? lastDate.getDate() : `0${lastDate.getDate()}`}/${lastDate.getFullYear()}&orderStatus=awaiting_shipping&page=${1}${id ? `&orderNumber=${id}` : ""}&pageSize=500`, headers).catch(e=>{console.log(e.response.data)})
    ...
    for(let i = 2; i <= parseInt(res?.data?.pages); i++){
        ...
        let res = await axios.get(`https://ssapi.shipstation.com/orders?createDateStart=${(lastDate.getMonth() + 1).toString().length == 2 ? lastDate.getMonth() + 1 : `0${lastDate.getMonth() + 1}`}/${lastDate.getDate().toString().length == 2 ? lastDate.getDate() : `0${lastDate.getDate()}`}/${lastDate.getFullYear()}&orderStatus=awaiting_shipping&page=${i}${id ? `&orderNumber=${id}` : ""}&pageSize=500`, headers).catch(e=>{console.log(e.response.data)})
