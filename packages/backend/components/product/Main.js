...
export const ProductsMain = ({prods, co, pa, blanks, seasons, genders, sportsUsedFor, brands, marketplaces, colors, themes, query, filter, CreateSku, source, totalProducts, printTypes, licenses, canManageMarketplaces, searchUrl, orgType, orgId }) => {
    ...
    return (
        <Box sx={{width: "100%", maxWidth: "100%", overflowX: "hidden"}}>
            <Container maxWidth="lg" sx={{minHeight: "90vh"}}>
                ...
                <MarketplaceModal open={marketplaceModal} setOpen={setMarketplaceModal} product={selectedProduct} setProduct={setSelectedProduct} marketPlaces={market} setMarketPlaces={setMarketPlaces} sizes={blanks.map(b => {return b.sizes.map(s => {return s.name})})} design={des} setDesign={setDesign} source={source} orgId={orgId} setProducts={setProducts} products={products} canManage={canManageMarketplaces} canEdit={canManageMarketplaces} />
                <CreateNFProduct open={NFProduct} setOpen={setNFProduct} stage={start} setStage={setStart} product={selectedProduct} setProduct={setSelectedProduct} setProducts={setProducts} brands={bran} setBrands={setBrands} seasons={seas} setSeasons={setSeasons} genders={gen} setGenders={setGenders} CreateSku={CreateSku} themes={them} setThemes={setThemes} sportUsedFor={sport} setSportUsedFor={setSportUsedFor} orgType={orgType} />
            </Container>
            <Footer />
