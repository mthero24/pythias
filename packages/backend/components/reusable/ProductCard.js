...
export const ProductCard = ({ p, setProduct, setCreateProduct, setNFProduct, marketPlaces, setMarketplaceModal, setStart, des, setDesign, updateDesign, setPreview, source, printTypes, licenses }) => {
        ...
        <Grid2 size={{ xs: 6, sm: 4 }}>
            <Box sx={{ padding: "2%", background: "#fff", boxShadow: "0px 0px 10px rgba(0,0,0,.1)", borderRadius: "5px", marginBottom: "2%" }}>
                ...
                <Box sx={{ display: "flex", flexDirection: "row", justifyContent: "center", alignItems: "center" }}>
                    <img src={p?.productImages?.filter(i => i.color?._id?.toString() == (p.defaultColor ? p.defaultColor?._id ? p.defaultColor?._id?.toString() : p.defaultColor?.toString() : p.colors[0]?._id?.toString()) && i.side != "back")[0]?.image} width={300} height={300} style={{ objectFit: "contain", borderRadius: "5px", width: "100%" }} />
                </Box>
                {p.productImages?.filter(i => i.color?._id?.toString() == (p.defaultColor ? p.defaultColor?._id ? p.defaultColor?._id?.toString() : p.defaultColor?.toString() : p.colors[0]?._id?.toString()) && (i.side == "back" || i.side == "modelBack"))[0] && <Box sx={{ display: "flex", flexDirection: "row", justifyContent: "flex-start", alignItems: "center", position: "relative", zIndex: 1, top: -130, left: 0, padding: "2%", marginBottom: "-130px", }}>
                    <img src={p.productImages.filter(i => i.color?._id?.toString() == (p.defaultColor ? p.defaultColor?._id ? p.defaultColor?._id?.toString() : p.defaultColor?.toString() : p.colors[0]?._id?.toString()) && (i.side == "back" || i.side == "modelBack"))[0]?.image} width={120} height={120} style={{ objectFit: "contain", borderRadius: "100px",  }} />
                </Box>}
