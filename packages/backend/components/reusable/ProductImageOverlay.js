...
const ProductImageOverlay = ({
  ...
  useEffect(() => {
    let getRender = async ()=>{
      ...
      setBase64(`https://imperial.pythiastechnologies.com/api/renderImages/${style}-${side}.jpg?blank=${style}&blankImage=${styleImage}&side=${side}&colorName=${colorName.value? colorName.value: colorName}&design=${designImage}&width=400&v=${Date.now()}`);
    }
    getRender()
