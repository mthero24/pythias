                                ...
                                product.blanks.map(b => {
                                    if (product.threadColors.length > 0) {
                                        ...
                                        for (let ti of Object.keys(design.images ? design.images : {})) {
                                            for (let col of product.colors) {
                                                if (b.images && b.images.length > 0) {
                                                    for (let bm of b.images.filter(m => m.color.toString() == col._id.toString() && Object.keys(m.boxes ? m.boxes : {}).includes(ti))) {
                                                        if (!im.filter(i => i.image == encodeURI(`https://${source == "pythias-test" ? "test" : source}.pythiastechnologies.com/api/renderImages/${design.sku}-${b.code.replace(/-/g, "_")}-${bm.image.split("/")[bm.image.split("/").length - 1].split(".")[0]}-${col.name.replace(/\//g, "_")}-${Object.keys(design.images ? design.images : {}).join("_") }.jpg}?width=400`)).length > 0) {
