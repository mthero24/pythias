                ...
                {stage == "blanks" && <Grid2 container spacing={2} sx={{ marginBottom: "2%" }}> 
                    ...
                    {blanks.map(b => {
                        ...
                        if (b.multiImages["modelFront"] && b.multiImages["modelFront"].length > 0) {
                            b.multiImages["front"] = b.multiImages["front"].concat(b.multiImages["modelFront"])
                        }
                        if (b.multiImages["modelBack"] && b.multiImages["modelBack"].length > 0) {
                            b.multiImages["back"] = b.multiImages["back"].concat(b.multiImages["modelBack"])
                        }
                        ...
                        <Card sx={{ padding: "2%", marginBottom: "2%" }}>
                            ...
                            <Box>
                                ...
                                <Grid2 container spacing={2} sx={{marginTop: "2%"}}>
                                    {
                                        cols.map(c=>(
                                            <Grid2 key={c._id} size={{xs: 3, sm: 1.5, md: 1, lg: .75, xl: .5}} sx={{"&:hover": {cursor: 'pointer', opacity: .6}}} onClick={()=>{
                                                let p = {...product}
                        ...
                        <Grid2 container spacing={2} sx={{ padding: "2%" }}>
                            ...
                            <Grid2 size={6}>
                                <Button fullWidth sx={{ margin: "1% 2%", background: "#645D5B", color: "#ffffff" }} onClick={async () => {
                                                ...
                                                for(let color of product.colors){
                                                    if(blank.colors.filter(c => c._id.toString() == color._id.toString())[0]){
                                                        for(let size of product.sizes){
                                                            ...
                                                            variants[blank.code][d][color.name].push({ image: img, size: size, color: color, sku: await CreateSku({ blank, color, size, design, threadColor: d }), threadColor: colors.filter(tc=> tc.name == d)[0], blank: blank })
                                                        }
                                                    }
                                                }
                                            }
                                    ...
                                    console.log(variants, "variants to set")
                                    let p = { ...product }
                                    p.variants = variants
...
const CreateVariantImages = ({ product, setProduct, design, threadColors, source }) => {
    let imgs = {}
    console.log(threadColors, "thread colors", design)
    if (!threadColors) {
